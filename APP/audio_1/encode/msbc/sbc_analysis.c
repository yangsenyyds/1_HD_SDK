...
{\
    __asm\
    {\
        MULS s32Temp,WIND_8_SUBBANDS_0_1,(s16X[ChOffset+16]-s16X[ChOffset+64]);\
        MULS s32Temp2,WIND_8_SUBBANDS_0_2,(s16X[ChOffset+32]-s16X[ChOffset+48]);\
        ADDS s32Temp,s32Temp,s32Temp2;\
        MOV s32DCTY[0],s32Temp;\
    }\
}
#endif
...
void SbcAnalysisFilter8 (SBC_ENC_PARAMS *pstrEncParams)
{
    ...
    s32NumOfChannels = 1; // pstrEncParams->s16NumOfChannels;
    ...
    for (s32Blk=0; s32Blk <15; /*s32NumOfBlocks;*/ s32Blk++)
    {
