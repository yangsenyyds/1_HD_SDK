...
static const uint8_t get_caps_respone_buf[] = {
    0X0B ,0X01 ,0X00 ,0X02 ,0X03 ,0X00 ,0XA0 ,0X01 ,0X00
};
#endif
...
static void Voice_Send(void)
{
    while(IPC_TxBufferIsEnough(4) && Bt_HciGetUsedBufferNum() < HCI_QUEUE_MAX -2 && VoiceEncodeQueMgr.current_queue_len > 0)
    {
        ...
        IWDG_ReloadCounter(&gWdtInit);
            ...
            ATT_sendNotify(AUDIO_SNED_HANDLE, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index].VoiceEncodeBuf[0], AUDIO_SEND_NTF_SIZE);

            voice_status.loss_cnt--;
            VoiceEncodeQueMgr.read_index++;
...
static void Voice_Encode(void)
{
    if (Get_MicReocrd_Len() >= ENCODE_INPUT_SIZE && VoiceEncodeQueMgr.current_queue_len < ENCODE_CACHE_NUM)
    {
        for(uint8_t i = 0; i < ENCODE_TIMES_ONCE; i++)
        {
            ...
            Adpcm_FrameEncode_Google_TV_Audio((int16_t *)MicRecord.ReadPtr, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.write_index].VoiceEncodeBuf[3], &audioHeader, ENCODE_INPUT_SIZE/2);
