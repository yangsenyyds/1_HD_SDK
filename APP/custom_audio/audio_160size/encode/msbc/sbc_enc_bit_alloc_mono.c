...
void sbc_enc_bit_alloc_mono(SBC_ENC_PARAMS *pstrCodecParams)
{
    ...
    SINT32  s32NumOfSubBands = 8; //pstrCodecParams->s16NumOfSubBands;
...
//            {
                ps16GenTabPtr = (SINT16 *)
                    sbc_enc_as16Offset8[0]; //[pstrCodecParams->s16SamplingFreq]; rk- furthur assign -2
//            }
            for(s32Sb=0; s32Sb<8; /*s32NumOfSubBands;*/ s32Sb++)
            {
                if(pstrCodecParams->as16ScaleFactor[s32Sb] == 0) //if(pstrCodecParams->as16ScaleFactor[(s32Ch*s32NumOfSubBands)+s32Sb] == 0)
                    *(ps16GenBufPtr) = -5;
                else
                {
                    s32Loudness =
                        (SINT32)(pstrCodecParams->as16ScaleFactor[s32Sb] //(SINT32)(pstrCodecParams->as16ScaleFactor[(s32Ch*s32NumOfSubBands)+s32Sb]
                                                            - *ps16GenTabPtr);
                    if(s32Loudness > 0)
        ...
        s32BitCount = 26; //pstrCodecParams->s16BitPool;
        s32SliceCount = 0;
        do
        {
            s32BitSlice --;
        ...
        s32Sb=0;
        while( (s32BitCount > 0) && (s32Sb < 8 /*s32NumOfSubBands*/) )
        {
