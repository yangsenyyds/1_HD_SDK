...
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint16_t adpcm_sendbuf_offset;
...
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint8_t voice_step;
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint8_t voice_offset = 3;
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint8_t voice_value_start = 0;
...
#endif
#ifdef FACTORY_MODE
        uint8_t sendbuf[ENCODE_OUTPUT_SIZE + FACTORY_FRAME_LEN] = {FRAME_HEADER, RCU << 4 + PC, CMD_VOICE, 0x00, ENCODE_OUTPUT_SIZE, 0x00};
        memcpy((void *)&sendbuf[6], (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index++], ENCODE_OUTPUT_SIZE);
        ATT_sendNotify(DONGLE_SEND_HANDLE, sendbuf, ENCODE_OUTPUT_SIZE + FACTORY_FRAME_LEN);
...
#elif ADPCM
#ifdef SONY_200
        if (adpcm_sendbuf_offset >= 240)
        {
            ATT_sendNotify(AUDIO_SNED_HANDLE, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index++].VoiceEncodeBuf[adpcm_sendbuf_offset], 19);
            adpcm_sendbuf_offset = 0;
            VoiceEncodeQueMgr.current_queue_len = 0;
            voice_value_start = 0;
            // DEBUG_LOG_STRING("e4--\r\n\r\n");
        }
        else
        {
            if (((voice_value_start == 1) && (adpcm_sendbuf_offset < 80)) || ((voice_value_start == 2) && (adpcm_sendbuf_offset < 160)) ||
                ((voice_value_start == 3) && (adpcm_sendbuf_offset < 240)))
            {
                ATT_sendNotify(AUDIO_SNED_HANDLE, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index].VoiceEncodeBuf[adpcm_sendbuf_offset], 20);
                adpcm_sendbuf_offset += 20;
                // DEBUG_LOG_STRING("e5\r\n");
            }
            else
            {
...
{
    // DEBUG_LOG_STRING("ve:%d %d\r\n", Get_MicReocrd_Len(), VoiceEncodeQueMgr.current_queue_len);
...
#ifdef SONY_200
            Adpcm_FrameEncode_Google_TV_Audio((int16_t *)MicRecord.ReadPtr, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.write_index].VoiceEncodeBuf[voice_offset], &audioHeader, ENCODE_INPUT_SIZE / 2);
            if (voice_step == 0)
            {
                VoiceEncodeQue[VoiceEncodeQueMgr.write_index].VoiceEncodeBuf[0] = (audioHeader.adpcmVal.previous_predict_adpcm & 0xff00) >> 8;
                VoiceEncodeQue[VoiceEncodeQueMgr.write_index].VoiceEncodeBuf[1] = (audioHeader.adpcmVal.previous_predict_adpcm & 0xff);
                VoiceEncodeQue[VoiceEncodeQueMgr.write_index].VoiceEncodeBuf[2] = audioHeader.adpcmVal.tableIndex;
                voice_step++;
                voice_offset += 86;
                voice_value_start = 1;
                // DEBUG_LOG_STRING("e1--\r\n");
...
#ifdef FACTORY_MODE
    QSPI_ReadFlashData(0x100D, 1, &adc_outputmode);
    msbc_encoder_init();
#endif
...
void audio_start_timer(void)
{
    DEBUG_LOG_STRING("audio_start_timer!!!\r\n");
    swtimer_start(start_ntf_timernum, START_NTF_TIME, TIMER_START_ONCE);
}
