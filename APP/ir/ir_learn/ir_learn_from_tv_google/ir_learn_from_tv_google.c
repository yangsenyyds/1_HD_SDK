...
void ir_learn_data_fill(uint8_t *tv_ir_learn_data)
{
    ...
    DEBUG_LOG_STRING("write irparams.freq = %d irparams.len = %d irparams.repeat = %d irparams.repeat_stop = %d irparams.crc = %d\r\n",
                             irparams.freq,
                             irparams.len, irparams.repeat, irparams.repeat_stop, irparams.crc);
