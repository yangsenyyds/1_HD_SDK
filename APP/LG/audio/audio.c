...
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint8_t adpcm_encode_offset;
...
#ifdef LG
MEMORY_NOT_PROTECT_UNDER_LPM_ATT static uint8_t VoiceNTFBuf[AUDIO_NTF_BUFSIZE];
#else
...
static void Audio_AdcConfig(void)
{
    ...
    // adcparaminit.Adc_OutputMode = ADC_SingleEndOutputWithCapacitor;  
...
{
    while(IPC_TxBufferIsEnough(4) && Bt_HciGetUsedBufferNum() < HCI_QUEUE_MAX && VoiceEncodeQueMgr.current_queue_len > 0)
    {
#ifdef AUDIO_FACTORY_MODE
            ATT_sendNotify(AUDIO_SNED_HANDLE, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index++], ENCODE_OUTPUT_SIZE);
            ...
            voice_status.loss_cnt--;
#ifdef LG   
			ntf_buf_offset += ENCODE_OUTPUT_SIZE - 3 + ENCODE_OUPUT_SECOND_OFFSET;
            if (ntf_buf_offset >= AUDIO_NTF_BUFSIZE)
#else
        ...
        for(uint8_t i = 0; i < ENCODE_TIMES_ONCE; i++)
        {
            ...
            if (voice_status.mode == ENCODE_DATA)
            {
                ...
                if (ntf_buf_offset >= AUDIO_NTF_BUFSIZE)
                {
                    ntf_buf_offset = AUDIO_FRAME_HEADER_LEN;
                    update_voice_packet(VoiceNTFBuf);
                    // UART_SendDataFromBuff(AUDIO_UART, (void *)&VoiceEncodeQue[VoiceEncodeQueMgr.read_index++], ENCODE_OUTPUT_SIZE);
                    UART_SendDataFromBuff(AUDIO_UART, (void *)&VoiceNTFBuf[2], VoiceNTFBuf[1]);
                }
#endif
                VoiceEncodeQueMgr.current_queue_len--;
                if (VoiceEncodeQueMgr.read_index == ENCODE_CACHE_NUM - 1) {
