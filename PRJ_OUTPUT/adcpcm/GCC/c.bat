...
:: compile respin and c bin to flash.dat
@rem  "%RESPIN_ROM_UTIL_PATH%/geneep" -f -n -z -k "%LOCAL_PATH%\key.dat"

if "%input%"=="n" (
	@echo generate enc flash 
	"%RESPIN_ROM_UTIL_PATH%/geneep" -f -n -z  -k "%LOCAL_PATH%\key.dat
	goto end 
)
