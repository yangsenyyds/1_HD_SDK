...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x30, 0x00, 3, 57}, // POWER 1
    {0xFD, 0x01, 3, 57}, // INPUT  1
    {0x1E, 0x00, 8, 72}, // 1
    {0x1F, 0x00, 8, 72}, // 2
    {0x20, 0x00, 8, 72}, // 3
    ...
    {0xF6, 0x01, 3, 57}, // GREEN  16
    {0xF5, 0x01, 3, 57}, // YEELOW
    {0xF4, 0x01, 3, 57}, // BLUE
    {0x40, 0x00, 3, 57}, //MUNE
    {0x6F, 0x00, 3, 57}, // INFO

    {0x41, 0x00, 3, 57}, // OK   21
    {0x42, 0x00, 3, 57}, // UP
    {0x44, 0x00, 3, 57}, // LEFT
    {0x45, 0x00, 3, 57}, // RIGHT
    {0x43, 0x00, 3, 57}, // DOWN

    {0x24, 0x02, 3, 57}, //BACK  26
    {0xFF, 0x01, 3, 57}, // APS
    {0x23, 0x02, 3, 57}, //HOME
    {0xE9, 0x00, 3, 57}, // VOL+ 31
    {0x9C, 0x00, 3, 57}, //CH+


    {0x21, 0x02, 3, 57}, //VOICE
    {0xE2, 0x00, 3, 57}, // MUTE
    {0xEA, 0x00, 3, 57}, //VOL-
    {0x9D, 0x00, 3, 57}, //CH-
    {0xB1, 0x00, 3, 57}, //LLEFT

    {0xB0, 0x00, 3, 57}, // LRIGHT 36
    {0xB7, 0x00, 3, 57}, // MTS
    {0xB4, 0x00, 3, 57}, // ZANTING
    {0xCA, 0x01, 3, 57}, // APPS
    {0xB3, 0x00, 3, 57}, // NETF
...
{0x02 ,0x01 ,0x05 ,0x03 ,0x19 ,0x80 ,0x01 ,0x03 ,0x02 ,0x12, 0x18 
,0x13 ,0xFF ,0x46 ,0x00 ,0x00 ,0xFF ,0xFF ,0xFF ,0xFF, 0xFF ,0xFF 
,0x00 ,0xBF ,0x0D ,0xF3 ,0xFF ,0x32 ,0x4A ,0x36, 0x30};
static const uint8_t scan_rsp_data_buf[] = 
{0x10 ,0x09 ,0x53 ,0x6D ,0x61 ,0x72 ,0x74 ,0x20 ,0x54 
,0x56 ,0x20 ,0x52 ,0x65 ,0x6D ,0x6F ,0x74 ,0x65};
...
static void key_pressed_handle(void)
{
    ...
    if (key_pressed_num == 1 || key_pressed_num == 0)
    {
        ...
        {
            if(key_pressed_num == 1 && keynum == menu_Keynum)
            {
                key_pressed_time++;
                if(key_pressed_time == 30)
                {
                    key_pressed_time = 0;
                    if(bt_check_le_connected()){
                        led_on(LED_1,300,1800);
                        led_on(LED_2,300,1800);
                        Bt_ClearRemoteDevInfo();
                        Bt_ClearDeviceNvdataInfo();
                        start_adv(ADV_TYPE_NOMAL, 0x10, true);
                    }
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    {
        if (key_report->keynum_report_buf[OK__Row] == OK__Keynum && key_report->keynum_report_buf[Back_Row] == Back_Keynum) {
            if(!bt_check_le_connected()){
            ir_single_send(0x8E,2);
            }
        }
    }   
}
...
void Write_DataParse(const ATT_TABLE_TYPE *table, uint8_t *data, uint8_t len)
{
    ...
    else if(table->handle == 87 && !memcmp(tv_open_write_handle_79,data,sizeof(tv_open_write_handle_79))) {
        ATT_sendNotify(89, (void*)tv_open_reply_handle_79, sizeof(tv_open_reply_handle_79));
    }
