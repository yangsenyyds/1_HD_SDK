...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x44, 0x00, 2, 53}, // INPUT  1
    {0x43, 0x00, 2, 53}, // POWER 1
    {0x45, 0x00, 2, 53}, // 1
    {0x00, 0x00, 2, 53}, // 2
    {0x21, 0x02, 2, 53}, // 3
...
{
// 0x02 ,0x01 ,0x04 ,0x03 ,0x03 ,0x12 ,0x18 ,0x03 ,0x19 ,0x80 ,0x01 ,0x12 
// ,0x09 ,0x43 ,0x68 ,0x72 ,0x6F ,0x6D ,0x65 ,0x63 ,0x61 ,0x73 ,0x74 ,0x20 
// ,0x52 ,0x65 ,0x6D ,0x6F ,0x74 ,0x65
0x02 ,0x01 ,0x05 ,0x03 ,0x03 ,0x12 ,0x18 ,0x03 ,0x19 ,0x80 ,0x01 ,0x12 
,0x09 ,0x43 ,0x68 ,0x72 ,0x6F ,0x6D ,0x65 ,0x63 ,0x61 ,0x73 ,0x74 ,0x20 
,0x52 ,0x65 ,0x6D ,0x6F ,0x74 ,0x65
};
static uint8_t scan_rsp_data_buf[] = {
    0x09 ,0xFF ,0xFE ,0xFE ,0x00 ,0x00 ,0x30 ,0x20 ,0x00 ,0x00
};
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    if (key_pressed_num == 0)
    {
        if(get_ir_learn_state () && (keynum == INPUT_Keynum || keynum == Power__Keynum || keynum == MUTE_Keynum || keynum == VOL_Keynum || keynum == VOL__Keynum)) {
            ...
            ATT_sendNotify(97, (void*)learn_ble_send, sizeof(learn_ble_send));
    ...
    {
        key_pressed_time = 0;
        if (key_report->keynum_report_buf[BACK__Row] == BACK_Keynum && key_report->keynum_report_buf[HOME__Row] == HOME__Keynum) {
