...
static uint8_t ir_lisrt_timernum = 0xFF;
...
static bool SecretKey_Check(void)
{
    ...
    DEBUG_LOG_STRING("secretkey_SN: %x \r\n", (secretkey_SN[0] << 8) + secretkey_SN[1]);
...
static void ir_list_handle(void)
{
    // if(send_number > 5){
    //     ir_time_send(&t36[0]);
...
static void keyvalue_handle(key_report_t *key_report)
{
    ...
    else if (key_pressed_num == 1)
    {
        ...
        else if (!bt_check_le_connected()) /*unpaire 83 44paired 44*/
        {
            ...
            else if(keynum == APP_KEY_IDX_LIST)
            {
                ir_single_send(0x53, 2);
                set_key_press_state(true);
            }
            else if(keynum == APP_KEY_IDX_MOVES)
            {
                ir_single_send(0x49, 2);
                set_key_press_state(true);
            }
            else
            {
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        ir_lisrt_timernum = swtimer_add(ir_list_handle);
