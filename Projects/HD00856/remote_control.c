...
static const uint8_t adv_data_buf[] =
    {
        0x02, 0x01, 0x05, 0x03, 0x03, 0x12, 0x18, 0x0B, 0xFF, 0x0D, 0x00, 0x38, 0x38, 0x02, 0x00, 0x00, 0x01, 0xFF, 0x4E, 0x03, 0x19, 0x80, 0x01};

static const uint8_t scan_rsp_data_buf[] =
    {
        0x08, 0x09, 0x48, 0x69, 0x73, 0x65, 0x6E, 0x73, 0x65, 0x11, 0xFF, 0x0D, 0x00, 0xFF, 0xFF, 0x45, 0x52, 0x46, 0x33, 0x4E, 0x36, 0x39, 0x56, 0x31, 0x2E, 0x30, 0x38
...
static const uint8_t scan_rsp_data_buf[] =
    {
        0X08, 0X09, 0X48, 0X69, 0X73, 0X65, 0X6E, 0X73, 0X65, 0X11, 0XFF, 0X0D, 0X00, 0XFF, 0XFF, 0X45, 0X52, 0X46, 0X33, 0X53, 0X36, 0X39, 0X56, 0X31, 0X2E, 0X30, 0X31};
#endif
...
static void key_pressed_handle(void)
{
    ...
    if (key_pressed_num == 1 || key_pressed_num == 0)
    {
        if (!bt_check_le_connected())
        {
            ...
            {
                key_pressed_time++;
                DEBUG_LOG_STRING("BAT key_pressed_time: %d \r\n", key_pressed_time);
...
static void keyvalue_handle(key_report_t *key_report)
{
    ...
    else if (key_pressed_num == 1)
    {
        ...
        DEBUG_LOG_STRING("KEY [%d][%d][%d][%d][%d][%d][%d] \r\n", key_report->keynum_report_buf[0], key_report->keynum_report_buf[1], key_report->keynum_report_buf[2], key_report->keynum_report_buf[3], key_report->keynum_report_buf[4], key_report->keynum_report_buf[5], key_report->keynum_report_buf[6]);
        if (bt_check_le_connected() && encrypt_state)
        {
            uint8_t hid_send_buf[KeyBuf[keynum].key_send_len];
            memset((void *)hid_send_buf, 0, KeyBuf[keynum].key_send_len);
