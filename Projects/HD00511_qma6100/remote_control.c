...
void Action_After_Prepare_Sleep()
{
    GPIO_Init(IIC_SCL_PIN, GPIO_Mode_Out_High);
    GPIO_Init(IIC_SDA_PIN, GPIO_Mode_Out_High);
    GPIO_Init(AGAINST_LED_PIN, GPIO_Mode_Out_Low);  //LED

    GPIO_Init(LIGHT_OFF_PIN, GPIO_Mode_Out_Low);   //LIGHTEN    
    GPIO_Init(LIGHT_CHK_PIN,GPIO_Mode_Deinit); //LIGHTCHECK
    
    qma_int_mode();
    qma6100_power_down();
}

void Action_After_Enter_Deep_Sleep(void){
    // GPIO_Init(IIC_SCL_PIN, GPIO_Mode_Out_High);
    // GPIO_Init(IIC_SDA_PIN, GPIO_Mode_Out_High);
    // GPIO_Init(AGAINST_LED_PIN, GPIO_Mode_Out_Low);  //LED
...
static void led_handle(void)
{
    ...
    if(led_stop_cnt == LED_BLK_OFF)
    {
        ...
        swtimer_restart(led_timernum);
    }
}

static void move_backlight_handle(void)
{
    //增加感光器件的判断条件
    int ret = GPIO_ReadDataBit(MOUSE_INT_PIN);
    // DEBUG_LOG_STRING("ret----------------------------\n");
    if(ret)
    {
        qma6100_initialize();
        GPIO_Init(LIGHT_OFF_PIN,GPIO_Mode_Out_High);
        if (GPIO_ReadDataBit(LIGHT_CHK_PIN))
        {
            DEBUG_LOG_STRING("ReadDataBitGPIO_23 = %d\n",GPIO_ReadDataBit(LIGHT_CHK_PIN));
            // swtimer_stop(led_timernum);
            led_stop_cnt = 0;
            GPIO_Init(LIGHT_OFF_PIN,GPIO_Mode_Out_Low);
            GPIO_Init(AGAINST_LED_PIN, GPIO_Mode_Out_High);
            app_sleep_lock_set(APP_LOCK,true);

            swtimer_start(led_timernum, 100, TIMER_START_ONCE);
        }
    }
}
...
static void remote_control_reinit(void)
{
    ...
    led_reinit();
    voice_report_reinit();
    swtimer_start(_g_move_timer,100,TIMER_START_REPEAT);
}
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        _g_move_timer = swtimer_add(move_backlight_handle);
        if (!SecretKey_Check())
        {
        ...
        swtimer_start(_g_move_timer,10,TIMER_START_REPEAT);
    ...
    else
    {
        ...
        {
            wake_up_state++;
            if(GPIO_ReadDataBit(MOUSE_INT_PIN)) {
                app_sleep_lock_set(APP_LOCK,true);
                // swtimer_start(led_timernum,1000,TIMER_START_ONCE);
            }
            sleep_time_state = 0;
            // if(!GPIO_ReadDataBit(KEY_COL1_PIN) && !GPIO_ReadDataBit(KEY_COL2_PIN) 
            //     && !GPIO_ReadDataBit(KEY_COL3_PIN)&& !GPIO_ReadDataBit(KEY_COL4_PIN)
            //     && !GPIO_ReadDataBit(KEY_COL5_PIN)&& !GPIO_ReadDataBit(KEY_COL6_PIN)){
            //         app_sleep_lock_set(LATENCY_LOCK,true);
            // }
            remote_control_reinit();
