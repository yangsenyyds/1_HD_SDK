...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x00, 0x00, 0x00, 0, 0}, // 1
    {0x00, 0x00, 0x00, 0, 0}, // 
    {0x20, 0x00, 0x00, 3, 134}, // 
    {0x00, 0x00, 0x00, 0, 0}, // 
    {0x00, 0x00, 0x00, 0, 0}, // 
...
static const uint8_t scan_rsp_data_buf[] = {0x0A, 0x09, 0x58, 0x69, 0x61, 0x6F, 0x6D, 0x69, 0x20, 0x52, 0x43};
...
void action_after_mic_close(void){
    ...
    ATT_sendNotify(66, zero, sizeof(zero));
}
...
static void keyvalue_handle(key_report_t *key_report)
{
    ...
    DEBUG_LOG_STRING("keyvalue_handle mem_le_conn_rcv %d mem_alloc_big_cur_len%d 0x49c5 = %d\r\n", HREAD(mem_le_conn_rcv),HREAD(mem_alloc_big_cur_len),HREAD(mem_le_md_count));
    if (key_pressed_num == 0)
    {
...
#endif
            enter_low_sleep();
            DEBUG_LOG_STRING("keep connect mem_le_conn_rcv %d mem_alloc_big_cur_len%d 0x49c5 = %d\r\n", HREAD(mem_le_conn_rcv),HREAD(mem_alloc_big_cur_len),HREAD(mem_le_md_count));
        }
    }
}
