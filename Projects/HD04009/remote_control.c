...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x80, 0x00, 8, 57}, // GREEN  16
    {0x9C, 0x00, 2, 64}, // YEELOW
    {0xFF, 0xFF, 2, 64}, // BLUE
    {0x81, 0x00, 8, 57}, // HOME
    {0xE2, 0x00, 2, 64}, // INFO

    {0x9D, 0x00, 2, 64}, // OK   21
    {0x40, 0x00, 2, 64}, // UP
    {0xCF, 0x00, 2, 64}, // LEFT
    {0xFF, 0xFF, 2, 64}, // RIGHT
    {0xDA, 0x00, 2, 64}, // DOWN

    {0xA9, 0x00, 2, 64}, //BACK  26
    {0xD6, 0x00, 2, 64}, //MUNE
    {0xAB, 0x00, 2, 64}, //VOL+
    {0xFD, 0x00, 2, 64}, //VOICE
    {0xFF, 0x00, 2, 64}, //CH+

    {0xA2, 0x01, 2, 64}, // VOL- 31
    {0X87, 0x00, 2, 64}, // MUTE
};
#elif (Project == 4009)
static const uint8_t ir_data[] = {
...
,0x19 ,0x80 ,0x01 ,0x13 ,0xFF ,0x0D ,0x00 ,0xDC 
,0x72 ,0x23 ,0x6F ,0xEB ,0xF0 ,0x45 ,0x52 ,0x46 
,0x33 ,0x4D ,0x39 ,0x30 ,0xFF ,0xFF ,0x41
};
static const uint8_t scan_rsp_data_buf[] = 
{
0x08 ,0x09 ,0x53 ,0x6D ,0x61 ,0x72 ,0x74 ,0x52 
,0x43 ,0x0D ,0xFF ,0x0D ,0x00 ,0x43 ,0x4F ,0x4E 
...
static uint8_t voice_send[4] = {0x04,0x03,0x02,0x00};
...
static void key_pressed_handle(void)
{
    if (key_pressed_num == 1 || key_pressed_num == 0)
    {
        ...
        {
#if (Project == 4010)
            if(keynum == USER_Keynum || keynum == SET_Keynum){
                if(ir_state != 1){
                    ir_state = 1;
    ...
    else if(key_pressed_num == 2 && keynum == Back__Keynum && keynum_second == Home_Keynum)
    {
            key_pressed_time++;
...
#if   (Project == 4009)           
                ir_single_send(0x44,2);
#elif (Project == 4010)
            if(ir_state != 2) {
                ir_state = 2;
                ir_init(UPD6121G2_68, CUSTOM_00_BF_A);
            }
            ir_single_send(0x82,2);
#endif                
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    else if (key_pressed_num == 2)
    {
        ...
        if (key_report->keynum_report_buf[Back__Row] == Back__Keynum && key_report->keynum_report_buf[Home_Row] == Home_Keynum) {
            key_pressed_time = 0;
            keynum = Back__Keynum;
            keynum_second = Home_Keynum;
            swtimer_start(key_pressed_timernum, 1000, TIMER_START_ONCE);
        }
        else if(key_report->keynum_report_buf[Back__Row] == Back__Keynum && key_report->keynum_report_buf[OK_Row] == OK_Keynum){
            if(!bt_check_le_connected()){
#if   (Project == 4009)           
                ir_single_send(0xC0,2);
#elif (Project == 4010)
            if(ir_state != 2) {
                ir_state = 2;
                ir_init(UPD6121G2_68, CUSTOM_00_BF_A);
