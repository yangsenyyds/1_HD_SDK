...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x00, 0x00, 0x00, 3, 82}, // INPUT  1
    {0x30, 0x00, 0x00, 3, 82}, // POWER 1
    {0x01, 0x00, 0x00, 3, 82}, // 
    {0x42, 0x00, 0x00, 3, 82}, // 
    {0x00, 0x80, 0x00, 3, 82}, // 

    {0x00, 0x00, 0x00, 3, 82}, //6
    {0x00, 0x00, 0x04, 3, 82}, // 
    {0x00, 0x00, 0x6E, 8, 75}, // 
    {0x00, 0x00, 0x10, 3, 82}, // 
    {0x00, 0x00, 0x20, 3, 82}, // 

    {0x00, 0x08, 0x00, 3, 82}, //11
    {0x00, 0x00, 0x3E, 8, 75}, // 
    {0x00, 0x00, 0x40, 3, 82}, // 
    {0x20, 0x00, 0x00, 3, 82}, //
    {0x80, 0x00, 0x00, 3, 82}, // 

    {0x10, 0x00, 0x00, 3, 82}, // 16
    {0x00, 0x01, 0x00, 3, 82}, // 
    {0x40, 0x00, 0x00, 3, 82}, // 
    {0x04, 0x00, 0x00, 3, 82}, // 
    {0x00, 0x20, 0x00, 3, 82}, // 

    {0x00, 0x00, 0x08, 3, 82}, // 21
    {0x00, 0x40, 0x00, 3, 82}, // 
    {0x00, 0x02, 0x00, 3, 82}, // 
    {0x00, 0x04, 0x00, 3, 82}, // 
    {0x08, 0x00, 0x00, 3, 82}, //VIOCE

    {0x00, 0x00, 0x01, 3, 82}, // 26
    {0x00, 0x00, 0x02, 3, 82}, // 
    {0x02, 0x00, 0x00, 3, 82}, // 
    {0x00, 0x00, 0x44, 8, 75}, // 
    {0x00, 0x00, 0x40, 8, 75}, // 

    {0x00, 0x00, 0x1E, 8, 75}, // 31
    {0x00, 0x00, 0x1F, 8, 75}, // 
    {0x00, 0x00, 0x26, 8, 75}, // 
    {0x00, 0x00, 0x21, 8, 75}, // 
    {0x00, 0x00, 0x22, 8, 75}, // 

    {0x00, 0x00, 0x23, 8, 75}, // 36
    {0x00, 0x00, 0x24, 8, 75}, // 
    {0x00, 0x00, 0x25, 8, 75}, //                 
    {0x00, 0x00, 0x20, 8, 75}, // 
    {0x00, 0x00, 0x27, 8, 75}, //     
...
,0x19 ,0x80 ,0x01 ,0x0C ,0x09 ,0x57 ,0x4F ,0x57 
,0x20 ,0x74 ,0x76 ,0x2B ,0x20 ,0x52 ,0x43 ,0x55 
};
static const uint8_t scan_rsp_data_buf[] = 
{
0x0C ,0x09 ,0x57 ,0x4F ,0x57 ,0x20 ,0x74 ,0x76 
,0x2B ,0x20 ,0x52 ,0x43 ,0x55};
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    else if (key_pressed_num == 1)
    {
        ...
        if (bt_check_le_connected() && encrypt_state)
        {
            ...
            DEBUG_LOG_STRING("att send  [%x] [%x] [%x] [%d] \r\n",KeyBuf[keynum].keyvalue[0],KeyBuf[keynum].keyvalue[1],KeyBuf[keynum].keyvalue[2],KeyBuf[keynum].key_send_len);

            if (keynum == Voice_Keynum && !voice_key_state)
            {
                DEBUG_LOG_STRING("voice  \r\n");
                ATT_sendNotify(AUDIO_CTRL_HANDLE, (void *)hid_send_buf, 1);
                ATT_sendNotify(KeyBuf[keynum].handle, (void*)hid_send_buf, KeyBuf[keynum].key_send_len);
