...
{
0x02 ,0x01 ,0x05 ,0x03 ,0x19 ,0x80 ,0x01 ,0x05 ,0x02 ,0x12 ,0x18 ,0x0F ,0x18
};
static const uint8_t scan_rsp_data_buf[] = 
{
0x09 ,0x09 ,0x54 ,0x56 ,0x43 ,0x30 ,0x47 ,0x57 ,0x42 ,0x54
};
...
static void key_pressed_handle(void)
{
    ...
    if (key_pressed_num == 1 || key_pressed_num == 0)
    {
        ...
        if (keynum == OK__Keynum && keynum_second == Back_Keynum)
        {
            ir_comm_send(0x62);
            set_key_press_state(true);
        }
        else if(keynum == CH__keynum && keynum_second == Back_Keynum)
        {
            key_pressed_time++;
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    uint8_t bt_le_connected_state = bt_check_le_connected();
    if (key_pressed_num == 0)
    {
        if (bt_le_connected_state && encrypt_state)
        {
    ...
    else if (key_pressed_num == 2)
    {
        ...
        if (key_report->keynum_report_buf[OK__Row] == OK__Keynum && key_report->keynum_report_buf[Back_Row] == Back_Keynum) {
            if(!bt_le_connected_state) {
                keynum = OK__Keynum;
                keynum_second = Back_Keynum;
                swtimer_start(key_pressed_timernum, 1000, TIMER_START_ONCE);
            }
        }else if(key_report->keynum_report_buf[CH__Row] == CH__keynum && key_report->keynum_report_buf[Back_Row] == Back_Keynum){
            if(!bt_le_connected_state) {
                keynum = CH__keynum;
                keynum_second = Back_Keynum;
                swtimer_start(key_pressed_timernum, 1000, TIMER_START_ONCE);
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        ir_init(UPD6121G2_68, CUSTOM_40_BF_A);
