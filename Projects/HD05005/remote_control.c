...
enum
{
    ...
    Netflix_Keynum = 21,
    CONN_PARAM = 128,
    APP_KEY_IDX_HOME_Keynum = 13,
    APP_KEY_IDX_HOME_Row = 0,
    APP_KEY_IDX_PWR = 2,
    APP_KEY_IDX_BACK = 10,
    APP_KEY_IDX_BACK_ROW = 3,
    APP_KEY_IDX_OK = 7,
    APP_KEY_IDX_VOICE = 14,
};
#if LG22
...
static uint8_t special_key_pressed_num;
static uint8_t keynum_secnd;
static uint8_t keynum;
static uint8_t long_press_keynum;
static uint8_t long_press_state;
static uint8_t long_press_send_cnt;
...
static uint8_t long_press_timernum = 0xFF;
...
static uint8_t auto_cnt_number = 10;
static uint8_t GYRO_BUF_SIZE_number = 20;
    ...
    if (acc_in && gyro_in && gyro_out)
    {
        ...
        && (app_abs(delta_gyro[2]) < GYRO_RESOLUTION_THR_VALUE)) ||
            ...
            (app_abs(delta_acc[2]) > ((ACC_RESOLUTION_THR_VALUE_MAX))) &&
             ...
             (app_abs(delta_gyro[2]) > GYRO_RESOLUTION_THR_VALUE_MAX))) && auto_cnt++ > auto_cnt_number)
        {
            auto_cnt = 0; 
            ...
            DEBUG_LOG_STRING("adjusting cnt %d  gyro_raw_buf[0] = %d gyro_raw_buf[cnt][1] = %d gyro_raw_buf[cnt][2] = %d\n",cnt,delta_gyro[0],delta_gyro[1],delta_gyro[2]);
            if (cnt >= (GYRO_BUF_SIZE_number - 1))
            {
                for (i = 0; i < GYRO_BUF_SIZE_number; i++)
                {
                ...
                if(auto_cnt_number == 0) {
                    auto_cnt_number = 20;
                }
                if(GYRO_BUF_SIZE_number == 10){
                    GYRO_BUF_SIZE_number = 20;
                }
                cnt = 0;
            }
            else
            {
                DEBUG_LOG_STRING("adjusting cnt %d",cnt);
                cnt++;
            }
        }        
        else
...
static void long_press_handle(void)
{
    bool le_state = bt_check_le_connected();    
    if(!long_press_state){
        key_pressed_time++;
        DEBUG_LOG_STRING("key_pressed_time[%d] %d\r\n", key_pressed_time,keynum);
        if(key_pressed_time == 30)
        {
            DEBUG_LOG_STRING("keynum switchover \r\n");
            key_pressed_time = 0;

            if(keynum == Back_Keynum)
            {
                KeyBuf[keynum].keyvalue[0] = 0x09;//发送静音
            }
            else if(keynum == Source_Keynum){
                KeyBuf[keynum].keyvalue[0] = 0x43;//发送设置  
            }
            else if(keynum == Home_Keynum){
                KeyBuf[keynum].keyvalue[0] = 0x53;//list  
            }

            if(!le_state)
            {
                ir_comm_send(KeyBuf[keynum].keyvalue[0]);
                if(key_pressed_num)
                {
                    set_key_press_state(true);
                }
                swtimer_stop(long_press_timernum);
            }else{
                long_press_state = true;
                special_key_pressed_num = 1;
                if (!key_chk_ok)
                {
                    key_chk_ok = 1;
                    moving_frst = 0;
        ...
        if(key_pressed_num == 0) {
            ...
            if(keynum == Back_Keynum)
            {
                KeyBuf[keynum].keyvalue[0] = 0x28;
            }
            else if(keynum == Source_Keynum){
                KeyBuf[keynum].keyvalue[0] = 0x0B;            
            }
            else if(keynum == Home_Keynum){
                KeyBuf[keynum].keyvalue[0] = 0x7C;            
            }
            if(!le_state) {
                ir_single_send(KeyBuf[keynum].keyvalue[0], 0);
                swtimer_stop(long_press_timernum);
            }
            else
            {
        ...
        DEBUG_LOG_STRING("long_press_send_cnt %d\r\n",long_press_send_cnt);
        if(long_press_send_cnt++ < 3 && Bt_HciGetUsedBufferNum() <= 1) {
        }else{
            special_key_pressed_num = 0;
            long_press_send_cnt = 0;
            long_press_state = 0;
            swtimer_stop(long_press_timernum);
...
{
    if (key_pressed_num == 2 && keynum == APP_KEY_IDX_BACK && keynum_secnd == APP_KEY_IDX_HOME_Keynum)
    {
...
static void keyvalue_handle(key_report_t *key_report)
{
    ...
    if((keynum == Home_Keynum || keynum == Back_Keynum || keynum == Source_Keynum) && bt_check_le_connected()){
        special_key_pressed_num = 0;
        DEBUG_LOG_STRING("Home_Keynum[%d]\r\n", special_key_pressed_num);
    }else{
        special_key_pressed_num = key_pressed_num;
        DEBUG_LOG_STRING("Back_Keynum[%d]\r\n", special_key_pressed_num);        
    }
    ...
    else if (key_pressed_num == 1)
    {
        ...
        else if(keynum == Home_Keynum || keynum == Back_Keynum || keynum == Source_Keynum )
        {
            key_pressed_time = 0;            
            if(bt_check_le_connected()){
                app_sleep_lock_set(APP_LOCK, true);
            }
            swtimer_start(long_press_timernum, 100, TIMER_START_REPEAT);
        }
    ...
    else if (key_pressed_num == 2)
    {
        ...
        if (key_report->keynum_report_buf[APP_KEY_IDX_HOME_Row] == APP_KEY_IDX_HOME_Keynum&& key_report->keynum_report_buf[APP_KEY_IDX_BACK_ROW] == APP_KEY_IDX_BACK)
        {
            keynum = key_report->keynum_report_buf[APP_KEY_IDX_BACK_ROW];
            keynum_secnd = key_report->keynum_report_buf[APP_KEY_IDX_HOME_Row];
        }
        key_pressed_time = 0;
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        long_press_timernum = swtimer_add(long_press_handle);
