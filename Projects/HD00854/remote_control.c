...
{
0x02 ,0x01 ,0x05 ,0x05 ,0x09 ,0x42 ,0x31 ,0x36 
,0x43 ,0x03 ,0x19 ,0x80 ,0x01 ,0x03 ,0x02 ,0x12 
,0x18 ,0x04 ,0x0D ,0x04 ,0x05 ,0x00};
static const uint8_t scan_rsp_data_buf[] = 
{
0x03 ,0x19 ,0x80 ,0x01 ,0x05 ,0x09 ,0x42 ,0x31 
,0x36 ,0x43};
...
static void key_pressed_handle(void)
{
    ...
    {
        if (keynum == Back__Keynum && keynum_second == HOME__Keynum)
        {
            key_pressed_time++;
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    {
        key_pressed_time = 0;
        if (key_report->keynum_report_buf[Back__Row] == Back__Keynum && key_report->keynum_report_buf[HOME__Row] == HOME__Keynum) {
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        led_init();
        voice_report_init();
        ir_init(UPD6121G2_68, CUSTOM_06_FE_A);
