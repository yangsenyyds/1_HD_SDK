...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x30, 0x00, 0x00, 6, 52},// 1 power
    {0x88, 0x01, 0x00, 6, 52}, //disnep 
    {0x41, 0x00, 0x00, 6, 52}, //ok
    {0x42, 0x00, 0x00, 6, 52},//up
    {0xCF, 0x00, 0x00, 6, 52}, // voice

    {0x99, 0x01, 0x00, 6, 52}, //APPS
    {0x00, 0x00, 0x00, 0, 0},  
    {0x00, 0x00, 0x00, 0, 0},  
    {0x00, 0x00, 0x00, 0, 0},
    {0x00, 0x00, 0x00, 0, 0},//
    ...
    {0x00, 0x00, 0x00, 0, 0},  
    {0x24, 0x02, 0x00, 6, 52},//back
    {0x00, 0x00, 0x00, 0, 0},
    {0xB0, 0X00, 0x00, 6, 52},//prime vide

    {0x8E, 0x00, 0x00, 6, 52},//netflix
    {0x23, 0x02, 0x00, 6, 52},//O
    {0x00, 0x00, 0x00, 0, 0},  
    {0x00, 0x00, 0x00, 0, 0},
    {0x00, 0x00, 0x00, 0, 0}, //  

    {0xEA, 0x00, 0x00, 6, 52},//vol-
    {0xE9, 0x00, 0x00, 6, 52},//vol+
    {0x00, 0x00, 0x00, 0, 0},  
    {0x00, 0x00, 0x00, 0, 0},
    {0x00, 0x00, 0x00, 0, 0}, // 
    ...
    {0x44, 0x00, 0x00, 6, 52},//31zuo
    {0x00, 0x00, 0x00, 0, 0},  
    {0x96, 0x01, 0x00, 6, 52},//tv+
    {0x43, 0x00, 0x00, 6, 52},//DOWN
    {0x45, 0x00, 0x00, 6, 52},//you
...
static  uint8_t report_handle93_1_adcpcm_value[] = {0x27, 0x17, 0x32, 0xb9, 0xD7, 0x07, 0x31, 0x19, 0xa0, 0x24, 0x00, 0x00};
static  uint8_t report_handle96_1_adcpcm_value[] = {0x00,0x00,0x1C,0x00,0x14};
static  uint8_t report_handle98_1_adcpcm_value[] = {0x02};

static uint8_t voice_send_adpcm[4] = {0x04, 0x03, 0x02,0x10};
...
void action_after_mic_close(void)
{
    if(List_state == ADCPCM_MODE)
    {
        if(voice_send_adpcm[1]== 0x01)
        {
            voice_key_state = false;
            uint8_t zero[2];
            memset(zero, 0, sizeof(zero));
            ATT_sendNotify(154, zero, sizeof(zero));
        }
        else{
            voice_key_state = false;
            uint8_t zero[2];
        ...
        if(keynum != Power_Keynum)
        {
            if(List_state == ADCPCM_MODE)
            {
                ...
                if (keynum == Voice_Keynum && !voice_key_state)
                {
                    ...
                    uint8_t voice_send[6] = {0x21, 0x02, 0x00, 0x00, 0x00, 0x00};
                    ATT_sendNotify(ADCPCM_KEYNUM_HANDLE, (void*)voice_send, sizeof(voice_send));
        ...
        if (le_connected_state && encrypt_state)
        {
            ...
            if(List_state == ADCPCM_MODE)
            {
                if(keynum == Voice_Keynum && voice_send_state == true) {
                    ...
                    mic_close();
                    // swtimer_stop(vioce_send_timernum);
                    // ATT_sendNotify(ADCPCM_KEYNUM_HANDLE, (void*)hid_send_buf, 6);
                }
                else if(keynum != Voice_Keynum) {
                    ATT_sendNotify(KeyBuf[keynum].handle, (void*)hid_send_buf, KeyBuf[keynum].key_send_len);
                }
    ...
    else if (key_pressed_num == 1)
    {
        ...
        if (le_connected_state && encrypt_state)
        {
            ...
            if(List_state == MSBC_MODE)
            {
                memset((void *)hid_send_buf, 0, KeyBuf[keynum].key_send_len);
                memcpy((void *)hid_send_buf, (void *)KeyBuf[keynum].keyvalue, 3); 
                DEBUG_LOG_STRING("MSBC_MODE att send  [%x] [%x] [%d] [%x]\r\n",KeyBuf[keynum].keyvalue[0],KeyBuf[keynum].keyvalue[1],
                    KeyBuf[keynum].key_send_len, KeyBuf[keynum].keyvalue[2],KeyBuf[keynum].keyvalue[3],KeyBuf[keynum].keyvalue[4]);              
                if (keynum == Voice_Keynum)
                {
                    ATT_sendNotify(AUDIO_MIC_OPEN_BEFORE_HANDLE, (uint8_t *)mic_open_before_buf, sizeof(mic_open_before_buf));
                }
            ...
            else
            {
                ...
                if (keynum == Voice_Keynum && !voice_key_state)
                {
                    ...
                    app_sleep_lock_set(AUDIO_LOCK, true);
                    // uint8_t voice_send[6] = {0x21, 0x02, 0x00, 0x00, 0x00, 0x00};
                    // uint8_t voice_send[6] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
                    // uint8_t
                    voice_send_adpcm[3]++;
                    ATT_sendNotify(ADCPCM_KEYNUM_HANDLE, (void*)voice_send_adpcm, sizeof(voice_send_adpcm));
                    voice_key_state = true;
                    voice_send_state = true;
                    voice_status.search = true;
                    // voice_status_change();
...
void Read_Parse(const ATT_TABLE_TYPE *table)
{
    ...
    if (table->handle == 93)
    {
        DEBUG_LOG_STRING("READ 93\r\n");
        memcpy((void *)&senddata[1], report_handle93_1_adcpcm_value, sizeof(report_handle93_1_adcpcm_value));
        ATT_Send(senddata, sendlen + 1);
    }
    else if(table->handle == 96)
    {
        DEBUG_LOG_STRING("READ 96\r\n");
        memcpy((void *)&senddata[1], report_handle96_1_adcpcm_value, sizeof(report_handle96_1_adcpcm_value));
        ATT_Send(senddata, sendlen + 1);
    }
    else if(table->handle == 102)
    {
        DEBUG_LOG_STRING("READ 102\r\n");        
        ...
        if (table->handle == AUDIO_CMD_ADCPCM_HANDLE)
        {
            ...
            else
            {
                if (!memcmp(GET_CAPS, data, 1)) {
                    if(len == 6)
                    {
                        uint8_t get_caps_respone_buf[] = {0x0B ,0x01 ,0x00 ,0x03 ,0x03 ,0x00 ,0x78 ,0x00,0x00};
                        voice_send_adpcm[1] = data[5];
                        get_caps_respone_buf[4] = data[5];
