...
static const KeyBuf_TypeDef KeyBuf[] = {
    ...
    {0x30, 0x00, 5, 31}, // INPUT  1
    {0xE2, 0x00, 5, 31}, // POWER 1
    {0x89, 0x00, 5, 31}, // 1
    {0xA2, 0x01, 5, 31}, // 2
    {0x40, 0x00, 5, 31}, // 3
...
0x02 ,0x01 ,0x05 ,0x03 ,0x19 ,0x80 ,0x01 ,0x0E ,0x09 ,0x54 ,0x56 
,0x20 ,0x42 ,0x4C ,0x45 ,0x20 ,0x52 ,0x65 ,0x6D ,0x6F ,0x74 ,0x65 
,0x05 ,0xFF ,0x5D ,0x00 ,0x55 ,0x88
};
...
static void key_pressed_handle(void)
{
    ...
    {
        if (keynum == VOl_Keynum && keynum_second == VOL____Keynum)
        {
            key_pressed_time++;
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    {
        key_pressed_time = 0;
        if (key_report->keynum_report_buf[VOL__Row] == VOl_Keynum && key_report->keynum_report_buf[VOL____Row] == VOL____Keynum) {

            if(!bt_check_le_connected()){
                keynum = VOl_Keynum;
                keynum_second = VOL____Keynum;
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        led_init();
        voice_report_init();
        ir_init(UPD6121G_64, CUSTOM_80_7F_A);
