...
static uint8_t key_pressed_time;
...
static uint8_t long_press_time = 50;
static bool long_press_state;
static uint8_t long_press_send_cnt;
static const uint16_t power_irdata[] = {
    38000, //204,
    136, 
     450,450	,55	,166	,55	,166	,56	,166	,55	,54
	,55	,54	    ,55	,55	    ,55	,54	    ,55	,54	    ,56	,166
	,55	,166	,55	,166	,55	,54	    ,55	,54	    ,56	,55
	,55	,55	    ,56	,54	    ,55	,54	    ,55	,166	,55	,166
	,55	,55	    ,56	,54	    ,55	,166	,55	,166	,55	,166
	,55	,166	,56	,54	    ,55	,54	    ,55	,166	,55	,166
	,55	,55	    ,56	,54	    ,55	,54	    ,55 ,4668,
     450,450	,55	,166	,55	,166	,56	,166	,55	,54
	,55	,54	    ,55	,55	    ,55	,54	    ,55	,54	    ,56	,166
	,55	,166	,55	,166	,55	,54	    ,55	,54	    ,56	,55
	,55	,55	    ,56	,54	    ,55	,54	    ,55	,166	,55	,166
    ...
    if(HREADW(mem_le_slave_latency) != CONN_PARAM && bt_check_le_connected()){
        if(key_pressed_num == 0) {
            ...
            if(sleep_time_state == 600){
                ...
                DEBUG_LOG_STRING("low_power_handle\r\n");
                sleep_time_state = 0;
            }
        }else{
            sleep_time_state = 0;
...
static void param_accepted_handle(void) {
    if(key_pressed_num != 0) {
        ...
        if(HREADW(mem_le_slave_latency) == 0) {
            bt_send_le_disconnect(0x13);
            DEBUG_LOG_STRING("param_accepted_handle\r\n");
        }
    }
}
...
static void long_press_handle(void){
    ...
    if(!long_press_state){
        ...
        if(key_pressed_time == long_press_time)
        {
            ...
            {
                long_press_keynum = 0x0F;
            }
            else if(keynum == Pause_Keynum){
                long_press_keynum = 0xb9;
            }
            else if(keynum == Netflix_Keynum){
                long_press_keynum = 0xDF;            
            }
            else if(keynum == Home_Keynum){
                long_press_keynum = 0x4F;            
            }

            if(!le_state){
                ir_comm_send(long_press_keynum);
                if(key_pressed_num)
                {
                ...
                memcpy((void *)hid_send_buf, (void *)&long_press_keynum, 1);
        ...
        if(key_pressed_num == 0) {
            key_pressed_time = 0;
            if(!le_state){
                ir_single_send(keyvalue_buf[keynum], 0);
                swtimer_stop(long_press_timernum);
            }else{
                memset((void *)hid_send_buf, 0, sizeof(hid_send_buf));
        ...
        if(long_press_send_cnt++ < 3 && encrypt_state && Bt_HciGetUsedBufferNum() <= 1) {
            ATT_sendNotify(KEY_SEND_HANDLE, (void *)hid_send_buf, KEY_SEND_LEN);
        }else{
            memset((void *)hid_send_buf, 0, sizeof(hid_send_buf));
            ATT_sendNotify(KEY_SEND_HANDLE, (void *)hid_send_buf, KEY_SEND_LEN);
...
static void keyvalue_handle(key_report_t *key_report)
{
    ...
    if (key_pressed_num == 0)
    {
        if (keynum == Pwr_Keynum) {
            swtimer_stop(power_timernum);
                ...
                keynum != Back_Keynum && keynum != Netflix_Keynum && keynum != Pause_Keynum && keynum != Home_Keynum )
        {
            memset((void *)hid_send_buf, 0, sizeof(hid_send_buf));
            ATT_sendNotify(KEY_SEND_HANDLE, (void *)hid_send_buf, KEY_SEND_LEN);
    ...
    else if (key_pressed_num == 1)
    {
        ...
        else if(keynum == Back_Keynum || keynum == Netflix_Keynum ){
            key_pressed_time = 0;            
            long_press_time = 30;
            if(bt_check_le_connected()){
        ...
        else if(keynum == Pause_Keynum || keynum == Home_Keynum){
            key_pressed_time = 0;            
            long_press_time = 30;
            if(bt_check_le_connected()){
                app_sleep_lock_set(APP_LOCK, true);
        ...
        else if (bt_check_le_connected() && encrypt_state)
        {
            ...
            memcpy((void *)hid_send_buf, (void *)&keyvalue_buf[keynum], 1);
            ATT_sendNotify(KEY_SEND_HANDLE, (void *)hid_send_buf, KEY_SEND_LEN);
            DEBUG_LOG_STRING("ble send KeyNum [%d] KEY_SEND_LEN = [%d]\r\n", hid_send_buf[0],KEY_SEND_LEN);
            if (keynum == Voice_Keynum) {
                mic_open();
            }
            else {
...
void CONN_PARAM_ACCEPTED(void)
{
    ...
    if(le_slave_param == CONN_PARAM) {
        ...
        swtimer_stop(low_power_timernum);
        bt_send_le_disconnect(0x13);
        DEBUG_LOG_STRING("CONN_PARAM_ACCEPTED\r\n");
    }else {
    ...
    else
    {
        ...
        else
        {
            ...
            if(sleep_time_state > 3800){
                bt_send_le_disconnect(0x13);
                DEBUG_LOG_STRING("KEEP CONNECT 111\r\n");
            }
#endif  
            enter_low_sleep();
