...
{
0x02 ,0x01 ,0x05 ,0x03 ,0x19 ,0x80 ,0x01 ,0x03 ,0x03 ,0x12 ,0x18};
static const uint8_t scan_rsp_data_buf[] = 
{
0x0E ,0x09 ,0x54 ,0x56 ,0x20 ,0x42 ,0x4C ,0x45 ,0x20 ,0x52 ,0x65 ,0x6D ,0x6F ,0x74 ,0x65
};
...
static void key_pressed_handle(void)
{
    ...
    {
        if (keynum == VOL___Keynum && keynum_second == BACK__Keynum)
        {
            key_pressed_time++;
        ...
        }else if(keynum == VOL___Keynum && keynum_second == BACK__Keynum){
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    {
        key_pressed_time = 0;
        if (key_report->keynum_report_buf[VOL___Row] == VOL___Keynum && key_report->keynum_report_buf[BACK__Row] == BACK__Keynum) {
        ...
        else if(key_report->keynum_report_buf[OK___Row] == OK___Keynum && key_report->keynum_report_buf[BACK__Row] == BACK__Keynum) {
...
void app_init(void)
{
    if (!Lpm_GetWakeFlag())
    {
        ...
        led_init();
        voice_report_init();
        ir_init(UPD6121G2_68, CUSTOM_00_7F_A);
