...
static void led_handle(void){
    ...
    GPIO_Init(LIGHT_CHK_PIN,GPIO_Mode_Out_Low);
...
static void keyvalue_handle(key_report_t* key_report)
{
    ...
    else if (key_pressed_num == 1)
    {
...
#ifdef  LIGHT_LED_PIN
        GPIO_Init(LIGHT_CHK_PIN,GPIO_Mode_Out_High);
        ...
        {
            GPIO_Init(GPIO_34,GPIO_Mode_Out_Low);
        }
...
static void remote_control_reinit(void)
{
    ...
    led_reinit();
    voice_report_reinit();
    GPIO_Init(LIGHT_LED_PIN,GPIO_Mode_Deinit);
}
...
void Action_After_Prepare_Sleep(){
    GPIO_Init(LIGHT_LED_PIN,GPIO_Mode_Out_High);
}
